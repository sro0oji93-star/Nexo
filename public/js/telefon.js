// Telefonbestellung: Touch-Warenkorb + Kundenformular. Baut Item-Objekte EXAKT wie
// die Kunden-Kasse (cart.js) und Theken-Kasse (kasse.js), damit priceItems() greift.
// Kein Rabatt, keine manuellen Lieferkosten (Zone serverseitig), kein Payment-Selector.
(function () {
  ...
  function render() {
    ...
    if (!items.length) {
      ...
      items.forEach(function (it, idx) {
        var d = descOf(it);
        html += '<div class="kasse-line"><div class="nm"><strong>' + it.qty + 'x ' + esc(it.name) + '</strong>'
          + (d ? '<small>' + esc(d) + '</small>' : '')
    ...
    document.getElementById('kasseTotal').textContent = fmt(totals());
  }

  function pushItem(o) { o.qty = 1; items.push(o); render(); }

  // Identisch zu kasse.js collect() (Produkt-Optionen) – Night-Deal-Abholer wird serverseitig abgelehnt.
  function collect(btn) {
    ...
    if (btn.getAttribute('data-has-sizes')) {
      ...
      pushItem({ id: id, name: name, price: unit, size: size, extras: extras, menue: null, sauce: null, sauces: bowlSauces.length ? bowlSauces : null, bowlExtras: bowlExtras.length ? bowlExtras : null, box: null, chocos: chocoSel.length ? chocoSel : null, deal: null, pasta: null, note: '' });
      ...
      if (drink) {
        ...
        pushItem({ id: id, name: name, price: msize.price, size: msize, extras: [], menue: { drink: true }, sauce: null, sauces: null, box: null, chocos: null, deal: null, pasta: null, note: '' });
      } else {
        pushItem({ id: id, name: name, price: parseFloat(btn.getAttribute('data-price')), size: null, extras: [], menue: null, sauce: null, sauces: null, box: null, chocos: null, deal: null, pasta: null, note: '' });
      }
      ...
      pushItem({ id: id, name: name, price: sunit, size: ssize, extras: [], menue: smenue, sauce: null, sauces: null, box: null, chocos: null, deal: null, pasta: null, note: '' });
      ...
      pushItem({ id: id, name: name, price: tunit, size: tsize, extras: [], menue: null, sauce: tsauceEl.getAttribute('data-sauce'), sauces: null, box: null, chocos: null, deal: null, pasta: null, note: '' });
      ...
      pushItem({ id: id, name: name, price: parseFloat(btn.getAttribute('data-price')), size: null, extras: [], menue: null, sauce: null, sauces: null, box: null, chocos: csel, deal: null, pasta: null, note: '' });
      ...
      if (btn.getAttribute('data-deal-day') === '1') {
        ...
        var tops2 = [];
        var tcs2 = dbox.querySelectorAll('input[data-deal-topping]:checked');
        for (var ti2 = 0; ti2 < tcs2.length; ti2++) tops2.push(tcs2[ti2].value);
        if (tops2.length > 3) { toast('Maximal 3 Beläge'); return; }
        deal.choices.belaege = tops2;
      } else {
      ...
      pushItem({ id: id, name: name, price: parseFloat(btn.getAttribute('data-price')), size: null, extras: [], menue: null, sauce: null, sauces: null, box: null, chocos: null, deal: deal, pasta: null, note: '' });
      ...
      pushItem({ id: id, name: name, price: parseFloat(btn.getAttribute('data-price')), size: null, extras: [], menue: null, sauce: null, sauces: null, box: null, chocos: null, deal: null, pasta: pasta, note: '' });
      ...
      pushItem({ id: id, name: name, price: parseFloat(btn.getAttribute('data-price')), size: null, extras: [], menue: null, sauce: null, sauces: null, box: { slug: bboxSlug, choices: box }, chocos: null, deal: null, pasta: null, note: '' });
    } else {
      pushItem({ id: id, name: name, price: parseFloat(btn.getAttribute('data-price')), size: null, extras: [], menue: null, sauce: null, sauces: null, box: null, chocos: null, deal: null, pasta: null, note: '' });
    }
  }

  function csrf() { return window.TELEFON_CSRF || ''; }

  function findKunde() {
    var phone = val('telSearch') || val('telPhone');
    ...
    fetch('/admin/kasse/telefon/kunde?phone=' + encodeURIComponent(phone), { credentials: 'same-origin' })
      ...
      .then(function (j) {
        ...
        if (j.success && j.found) {
          ...
          setVal('telPhone', j.kunde.phone || phone);
        } else {
          foundKunde = null;
          box.style.display = '';
          box.textContent = 'Neuer Kunde – bitte Daten eingeben und speichern.';
          if (!val('telPhone')) setVal('telPhone', phone);
        }
      })
  ...
  function takeKunde() {
    ...
    setVal('telPhone', foundKunde.phone || '');
    setVal('telName', foundKunde.name || '');
    setVal('telStrasse', foundKunde.strasse || '');
    setVal('telHausnr', foundKunde.hausnummer || '');
    setVal('telPlz', foundKunde.plz || '');
    setVal('telOrt', foundKunde.ort || '');
    toast('Daten übernommen');
  }

  function saveKunde() {
    var body = { phone: val('telPhone'), name: val('telName'), strasse: val('telStrasse'), hausnummer: val('telHausnr'), plz: val('telPlz'), ort: val('telOrt') };
  ...
  function submit() {
    ...
    var body = {
      items: items,
      name: val('telName'), phone: val('telPhone'),
      strasse: val('telStrasse'), hausnummer: val('telHausnr'),
      plz: val('telPlz'), ort: val('telOrt'),
      notes: val('telNotes')
    };
      ...
      .then(function (j) {
        ...
        if (last) {
          ...
          if (j.id) {
            ...
            a.target = '_blank';
            a.textContent = 'Fahrerbon drucken';
            last.appendChild(a);
  ...
  document.addEventListener('click', function (e) {
    ...
    if (rm) { items.splice(parseInt(rm.getAttribute('data-kr'), 10), 1); render(); return; }
    if (e.target.closest && e.target.closest('#kasseClear')) { items = []; render(); return; }
